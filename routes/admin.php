...
Route::prefix('admin')->name('admin.')->middleware(['checkAuth','role:admin|teacher'])->group(function ()
{
    ...
    //Book Orders
    Route::resource('/book-orders', BookOrderController::class)->names('book-order');
    Route::post('/change-book-order-status', [BookOrderController::class, 'changeBookOrderStatus'])->name('book-order.status');
